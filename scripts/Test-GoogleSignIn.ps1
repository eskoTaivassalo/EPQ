...
if (Test-Path "$projectRoot\app.json") {
    ...
    if ($appJson.expo.android.package -eq "com.parents2teachers") {
        Write-Host "   ✅ Android package: com.parents2teachers" -ForegroundColor Green
    } else {
...
if (Test-Path "$projectRoot\src\services\authService.js") {
    ...
    if ($authService -match 'webClientId:\s*[''"]892513281177-v630fe7a65rpdoiqni3cpsqsk23qgb5u\.apps\.googleusercontent\.com[''"]') {
