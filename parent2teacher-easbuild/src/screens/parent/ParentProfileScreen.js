  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      setProfileData(prev => ({
        ...
        photoURL: merged.photoURL || prev.photoURL,
      }));
      setProfileImageUri(merged.photoURL || null);
    } catch (error) {
    ...
    try {
      ...
      if (profileImageUri && profileImageUri !== profileData.photoURL) {
        try {
          ...
          const photoURL = await AuthService.updateProfileImage(profileImageUri, user.uid, 'parent');
          updatePayload.photoURL = photoURL;
          updatePayload.profile.photoURL = photoURL;
          console.log('✅ Profile image updated');
        } catch (imageError) {
          console.error('❌ Error updating profile image:', imageError);
          Alert.alert('Notice', 'Profile image update failed, but other data was saved.');
        }
      } else if (profileData.photoURL) {
        // Keep existing photo URL if no change
        updatePayload.photoURL = profileData.photoURL;
        updatePayload.profile.photoURL = profileData.photoURL;
